...
rn = @reaction_network begin
    @parameters k_on k_off k_syn k_spl k_deg 
    (k_on,k_off), OFF <--> ON
    k_syn, ON --> ON + N
    k_spl, N --> M
    k_deg, M --> 0
...
if test_data_flag
    ...
    open("$root_path/test_$(model_name)_stable_$max_val_ds.json", "w") do f
        write(f, json_str)
    end
    df = DataFrame(parameters_matrix, :auto)
    CSV.write("$root_path/test_$(model_name)_stable_parameter_$data_size.csv", df)
    truncate_max = findmax(matrix_mean+3*matrix_std,dims=1)[1]
    CSV.write("$root_path/test_$(model_name)_truncate_max_$data_size.csv",DataFrame(truncate_max, :auto))
