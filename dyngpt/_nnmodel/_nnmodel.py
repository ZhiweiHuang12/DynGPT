...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.block_size = kwargs["variable_dimension"] + self.N
        # self.block_size = 68
...
class NNmodel(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.lm_head = nn.Linear(self.emb_dim, self.U, bias=False)  
    ...
    def forward(self, idx,targets = None,species_index=[]):
...
class NNmodel_FT(nn.Module):
    """
    transformer for estimating joint distribution of generalized cme
    """
    def __init__(self, *args, **kwargs):
        super(NNmodel_FT, self).__init__()
        self.N = kwargs["num_species"]
        self.M = kwargs["num_reactions"]
        # use a fixed prompt input, useful for combining models with different sizes
        self.var_dim = kwargs["variable_dimension"]  # dimension of the input prompt after perceptron
    ...
    def forward(self, idx):
        ...
        if self.constrains[0] == 0 or np.all(self.constrains == self.U):
            ...
            index11 = self.constrains[x.shape[1]-self.var_dim]
    ...
    def log_joint_prob(self, idx,filtered_species=[],log_flag=True,unobserved_number = 1):
        # assert idx.shape[1] == self.block_size, "joint probability can only be calculated for full sequence"
        b, t = idx.shape
        device = idx.device
        ljp = torch.zeros(b, self.N, device=device)
        ...
        if unobserved_number==0:
            ...
            row_products = torch.exp(ljp).prod(dim=1)
            n = ljp.size(0)  
            half_n = n // 2
            sums = row_products[:half_n] + row_products[half_n:]
            ...
            row_products = torch.exp(ljp).prod(dim=1)
            sums = row_products.view( 4,ljp.size(0) // 4).sum(dim=0)
            log_sums = torch.log(sums)
            if log_flag:
                return log_sums
            else:
    ...
    @torch.no_grad()
    def sample(self, idx,norm_sample=False):
        """
        Take a conditioning sequence of indices idx (LongTensor of shape (b,t)) and complete
        the sequence max_new_tokens times, feeding the predictions back into the model each time.
        Most likely you'll want to make sure to be in model.eval() mode of operation for this.
        ...
        for i in range(self.N):
            ...
            # if constrains is specified for each species
                ...
                if norm_sample:
                    cp = cp.detach().cpu()
                    cp = torch.where(cp < 1e-4, torch.tensor(0), torch.tensor(1))
                    prob_sum = cp.sum()
                    cp = cp / prob_sum
                    cp = cp.to(self.device)
                idx_next = torch.multinomial(cp, num_samples=1)
                samples[:, i] = idx_next[:, 0]
                idx = torch.cat((idx, idx_next), dim=1)
