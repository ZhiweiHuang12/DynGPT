...
# --------------------------------- model architecture -------------------------------------#
def get_sdp_model_config():
    from dyngpt.nnmodel.hyperparameters import args
    args.model = 'sdp'
    args.num_species = 2  # Species number
    args.num_reactions = 7  # reaction number
    args.num_init = 2 # Species number of init

    # Upper limit of the molecule number: it is adjustable and can be indicated by doing a few Gillespie simulation.
    # args.state_upper_bound = int(377)
    # args.constrains = np.array([150,377], dtype=int)# V1
    args.state_upper_bound = int(122)
    args.constrains = np.array([121,122], dtype=int) 
    # --------------------------------- train NNmodel -------------------------------------#
    args.bits = 1
